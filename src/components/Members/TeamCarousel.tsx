  ...
  return (
    ...
    >
      ...
      >
        {datalist.map((item, index) => (
          ...
          >
            <div
              className={cn(
                `flex h-full w-full flex-col items-center justify-center  bg-cover bg-center bg-no-repeat rounded-lg pt-14`,
                item.color
              )}
            >
              <img
                src={item.image}
                alt={"Team Logo"}
                className='absolute top-1/5 -translate-y-1/2 z-0 mb-2 h-32 w-32 object-contain opacity-20'
              />
          
              {item.members?.map((member, memberIndex) => (
                <p
                  key={memberIndex}
                  className={cn(`text-lightBlack/60 z-10 text-center text-xl italic`, 
                    isCaptain(member) ? "font-bold text-2xl" : "font-normal"
                  )
                  }
                  style={{
      ...
      <div className='absolute top-1/2 z-[40] flex w-full -translate-y-1/2 transform justify-between px-4'>
        <button
          onClick={handleNext}
          className='z-10 rounded-full bg-gray-300/30 p-2 shadow-md backdrop-blur-md duration-300 hover:bg-gray-400/30'
        >
          <CaretLeft size={32} />
        </button>
        <button
